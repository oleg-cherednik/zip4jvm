...
class DoubleFastBlockCompressor
    ...
    public int compressBlock(byte[] inputBase,
        ...
        while (input < inputLimit) {   // < instead of <=, because repcode check at (input+1)
            ...
            if (offset1 > 0 && UnsafeUtil.getInt(inputBase, input + 1 - offset1) == UnsafeUtil.getInt(inputBase,
                                                                                                      input + 1)) {
                ...
                if (longMatchAddress > windowBaseAddress && UnsafeUtil.getLong(inputBase, longMatchAddress) ==
                        UnsafeUtil.getLong(inputBase, input)) {
                    matchLength = count(inputBase, input + SIZE_OF_LONG, inputEnd, longMatchAddress + SIZE_OF_LONG) +
                            SIZE_OF_LONG;
                    offset = (int) (input - longMatchAddress);
                    while (input > anchor && longMatchAddress > windowBaseAddress && UnsafeUtil.getByte(inputBase,
                                                                                                        input - 1) ==
                            UnsafeUtil.getByte(inputBase, longMatchAddress - 1)) {
                        input--;
                        longMatchAddress--;
                        matchLength++;
                    }
                    ...
                    if (shortMatchAddress > windowBaseAddress && UnsafeUtil.getInt(inputBase, shortMatchAddress) ==
                            UnsafeUtil.getInt(inputBase, input)) {
                        ...
                        if (nextOffsetMatchAddress > windowBaseAddress && UnsafeUtil.getLong(inputBase,
                                                                                             nextOffsetMatchAddress) ==
                                UnsafeUtil.getLong(inputBase, input + 1)) {
                            matchLength = count(inputBase,
                            ...
                            while (input > anchor && nextOffsetMatchAddress > windowBaseAddress && UnsafeUtil.getByte(
                                    inputBase,
                                    input - 1) == UnsafeUtil.getByte(inputBase, nextOffsetMatchAddress - 1)) {
                                input--;
                                nextOffsetMatchAddress--;
                                matchLength++;
                            }
                            ...
                            while (input > anchor && shortMatchAddress > windowBaseAddress && UnsafeUtil.getByte(
                                    inputBase,
                                    input - 1) == UnsafeUtil.getByte(inputBase, shortMatchAddress - 1)) {
                                input--;
                                shortMatchAddress--;
                                matchLength++;
                            }
                ...
                output.storeSequence(inputBase,
                                     anchor,
                                     (int) (input - anchor),
                                     offset + REP_MOVE,
            ...
            if (input <= inputLimit) {
                // Fill Table
                longHashTable[hash8(UnsafeUtil.getLong(inputBase, baseAddress + current + 2), longHashBits)] =
                        current + 2;
                shortHashTable[hash(inputBase, baseAddress + current + 2, shortHashBits, matchSearchLength)] =
                        current + 2;

                longHashTable[hash8(UnsafeUtil.getLong(inputBase, input - 2), longHashBits)] = (int) (input - 2 -
                        baseAddress);
                shortHashTable[hash(inputBase, input - 2, shortHashBits, matchSearchLength)] = (int) (input - 2 -
                        baseAddress);

                while (input <= inputLimit && offset2 > 0 && UnsafeUtil.getInt(inputBase, input) == UnsafeUtil.getInt(
                        inputBase,
                        input - offset2)) {
                    int repetitionLength = count(inputBase,
                                                 input + SIZE_OF_INT,
                                                 inputEnd,
                    ...
                    shortHashTable[hash(inputBase, input, shortHashBits, matchSearchLength)] = (int) (input -
                            baseAddress);
                    longHashTable[hash8(UnsafeUtil.getLong(inputBase, input), longHashBits)] = (int) (input -
                            baseAddress);
