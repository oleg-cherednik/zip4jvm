...
class FiniteStateEntropy {
    ...
    private static int compress1(ByteArrayWithOffs out,
        ...
        if ((inputSize & 1) != 0) {
            input--;
            state1 = table.begin(UnsafeUtil.getByte(in, input));

            input--;
            state2 = table.begin(UnsafeUtil.getByte(in, input));

            input--;
            state1 = table.encode(stream, state1, UnsafeUtil.getByte(in, input));
        ...
        if ((SIZE_OF_LONG * 8 > MAX_TABLE_LOG * 4 + 7) && (inputSize & 2) != 0) {  /* test bit 2 */
            input--;
            state2 = table.encode(stream, state2, UnsafeUtil.getByte(in, input));
    ...
    public static int optimalTableLog(int maxTableLog, int inputSize, int maxSymbol) {
        ...
        result = Math.min(result,
                          Util.highestBit((inputSize - 1)) - 2); // we may be able to reduce accuracy if input is small
    ...
    private static int normalizeCounts2(short[] normalizedCounts,
                                        int tableLog,
                                        int[] counts,
                                        int total,
                                        int maxSymbol) {
        ...
        int lowThreshold = total >>>
                tableLog; // minimum count below which frequency in the normalized table is "too small" (~ < 1)
        int lowOne = (total * 3) >>>
                (tableLog + 1); // 1.5 * lowThreshold. If count in (lowThreshold, lowOne] => assign frequency 1
