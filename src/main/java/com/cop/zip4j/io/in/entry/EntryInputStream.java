...
public abstract class EntryInputStream extends InputStream {
    ...
    public static InputStream create(@NonNull CentralDirectory.FileHeader fileHeader, char[] password, DataInput in, UnzipEngine engine)
            throws IOException {
        long pos = in.getOffs();
        ...
        if (compression == Compression.STORE)
            return new ZipInputStream(new PartInputStream(in, comprSize, decoder, engine), fileHeader, decoder);
        if (compression == Compression.DEFLATE)
            return new ZipInputStream(new InflaterInputStream(in, comprSize, decoder, engine, fileHeader), fileHeader, decoder);
