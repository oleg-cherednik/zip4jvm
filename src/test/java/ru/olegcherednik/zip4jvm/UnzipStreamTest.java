...
public class UnzipStreamTest {
    ...
    private static final Path rootDir = Zip4jvmSuite.rootDir.resolve(UnzipStreamTest.class.getSimpleName());
    ...
    public void shouldUnzipEntryToStreamWhenNoSplit() throws IOException {
        ...
        ZipFile.Reader zipFile = ZipFile.read(Zip4jvmSuite.deflateSolidZip);
    ...
    public void shouldUnzipEntryToStreamWhenPkwareNoSplit() throws IOException {
        Path imgFile = rootDir.resolve("bentley-continental.jpg");
        ZipFile.Reader zipFile = ZipFile.read(Zip4jvmSuite.deflateSolidPkwareZip, fileName -> Zip4jvmSuite.password);
        TestUtils.copyLarge(zipFile.extract("cars/bentley-continental.jpg").getInputStream(), imgFile);
        Zip4jvmAssertions.assertThatFile(imgFile).exists().isImage().hasSize(1_395_362);
    }
}
